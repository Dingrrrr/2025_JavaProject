...
public class AlbumMainScreen2 extends JFrame {
	...
	private void addAlbum() {
		...
		albumLabel.setPreferredSize(new Dimension(170, 100)); // 크기 고정
		albumLabel.setMaximumSize(new Dimension(170, 100)); // 크기 고정

		// 태그 레이블 생성
		JLabel tagLabel = new JLabel("태그");
		tagLabel.setHorizontalAlignment(SwingConstants.CENTER);
		tagLabel.setPreferredSize(new Dimension(170, 20)); // 크기 고정
		tagLabel.setOpaque(true);
		tagLabel.setBackground(Color.LIGHT_GRAY);

		// 태그 레이블 생성
		JTextArea tagTextArea = new JTextArea("");
		tagTextArea.setPreferredSize(new Dimension(169, 30)); // 크기 고정
		tagTextArea.setOpaque(true);
		tagTextArea.setEditable(false);
		tagTextArea.setBackground(Color.LIGHT_GRAY);
		...
		albumWithTagPanel.add(tagTextArea);

		// 앨범 + 태그 패널 크기 고정
		albumWithTagPanel.setPreferredSize(new Dimension(176, 150)); // 앨범과 태그 합친 크기

		// 테두리 추가
		albumWithTagPanel.setBorder(new LineBorder(Color.lightGray, 1)); // 검정색 2픽셀 두께의 테두리 추가
		...
		albumPanel.setPreferredSize(new Dimension(338, rows * 130)); // 크기 유지
